    ...
    <div className="relative min-h-screen bg-gradient-subtle">
      ...
      {!isMobile && (
        <>
          <aside
            className={cn(
              "fixed inset-y-0 left-0 z-40 hidden flex-col border-r border-border/50 bg-card/95 backdrop-blur-xl transition-all duration-300 lg:flex",
          ...
          >
            ...
            >
              {!sidebarCollapsed && (
                <Link href="/app" className="flex items-center gap-2">
                  ...
                  <span className="font-semibold tracking-tight">CleanOPS</span>
                </Link>
              )}
              {sidebarCollapsed && (
                <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-primary text-primary-foreground font-bold text-sm">
            ...
            <div
              className={cn(
                "border-t border-border/50 p-3 space-y-2",
                sidebarCollapsed && "flex flex-col items-center"
              )}
            >
              {!sidebarCollapsed && <OnlineStatusPill className="w-full justify-center" />}
              {sidebarCollapsed ? (
                <Button
                  variant="ghost"
                  size="icon"
            ...
            {sidebarOpen && (
              <>
                ...
                <motion.aside
                  ...
                  className="fixed inset-y-0 left-0 z-50 w-72 border-r border-border/50 bg-card shadow-2xl lg:hidden"
                >
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  className="relative h-10 w-10 rounded-full ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                >
              ...
              <DropdownMenuContent align="end" className="w-56">
                ...
                <SignOutButton
                  ...
                  size="sm"
                  showIcon={true}
                  className="w-full justify-start px-2 py-1.5 text-sm font-normal text-destructive hover:text-destructive hover:bg-destructive/10 rounded-sm"
                />
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
      ...
      {isMobile && (
        <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-border/50 bg-background/95 backdrop-blur-xl" style={{ paddingBottom: "calc(0.5rem + env(safe-area-inset-bottom, 0px))" }}>
            ...
            {navItems.slice(0, 5).map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href as Route}
                  className={cn(
                    "flex min-w-[60px] flex-col items-center gap-1 rounded-xl px-2 py-1.5 text-[11px] font-medium transition-colors",
                    isActive
                      ? "text-primary"
